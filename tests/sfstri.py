...
def testOrientableSFS():
    testFibres = [
            ...
            [ (2,1), (5,1), (5,-2) ],
            [ (3,1), (7,-2), (7,2), (7,-3) ] ]
    #NOTE Regina is not always consistent with how it chooses from the many
    #       alternative names that these manifolds have.
    expectedNames = [
            "RP3 # RP3",
            "SFS [S2: (2,1) (2,1) (2,3)]",
            "SFS [RP2/n2: (3,1) (4,-1)]",
...
def testOrientableBundle():
    # Check that we get oriented triangulations of the correct 3-manifolds.
    #
    #NOTE Regina is not always consistent with how it chooses from the many
    #       alternative names that these manifolds have.
    expectedNames = [
...
def testTriPrism():
    ...
    for isSolidTorus in ( True, False ):
        ...
        _testTriPrismBasic( description, tri, isSolidTorus )
        count += 1
        ...
        for _ in range(2):
            for s in range(3):
                _testTriPrismSlopeSign( prism, s, tri, isSolidTorus )
                count += 1
    return count


def _testTriPrismBasic( description, tri, isSolidTorus ):
    """
...
def _testTriPrismSlopeSign( prism, s, tri, isSolidTorus ):
    """
    Test that the slope-sign constraint holds both before and after flipping
    square s of the given prism.
    """
    if isSolidTorus:
    ...
    doTest( slopeDesc + " New slope.",
            -oldSlope, prism.squareSlope(s) )
    _testTriPrismBasic( description + " After flip.", tri, isSolidTorus )
    for t in range(2):
...
if __name__ == "__main__":
    ...
    if "sfs" in testNames:
        longName = "orientableSFS( baseSignedGenus, boundaries, *fibres )"
        runNamedTestSuite( longName, testOrientableSFS )

    # Test OrientableBundle class.
    if "bundle" in testNames:
        longName = "OrientableBundle class"
        runNamedTestSuite( longName, testOrientableBundle )

    # Test TriPrism class.
    if "prism" in testNames:
        longName = "TriPrism class"
        runNamedTestSuite( longName, testTriPrism )
